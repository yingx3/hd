function[Uw,Us]=area_y(Uw,Us,dt)
    ...
    Fa=y_HLLC(h_alefc,u_alef,v_alef,h_arigc,u_arig,v_arig);
    ...
    Fb=y_HLLC(h_blefc,u_blef,v_blef,h_brigc,u_brig,v_brig);

    A=0.*Fa;B=0.*Fb;
    A(:,:,3)=1/2.*g.*(h_arig.^2-h_arigc.^2);
    B(:,:,3)=1/2.*g.*(h_blef.^2-h_blefc.^2);

    Us=Us-dt/dy.*(Fb-Fa+B-A);
